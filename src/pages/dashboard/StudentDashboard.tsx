...
import { BookOpen, User, Calendar, Search, ChevronDown, Star, ArrowRight, Gift, Video, Monitor, Mail, Apple, RefreshCw, IndianRupee, CreditCard } from "lucide-react";
...
// Dynamic filter lists will be derived from the advisors directory
...
function BookingCardContent({ booking }: { booking: BookingResponse }) {
  ...
  const handleSyncStatus = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        window.location.reload(); // Refresh to show confirmed status
      } else {
  ...
  const getCalendarEvent = (): CalendarEvent => ({
    title: `Session with ${booking.advisor_name}`,
    description: `CollegeConnect Session\nJoin here: ${window.location.origin}/student/session/${booking.id}\nMeet Link: ${booking.meet_link || "Will be available 10 mins before"}`,
      ...
      <div className="mt-4 flex flex-col gap-2">
        <div className="flex gap-2">
          <Button
            ...
            className="flex-1 gap-1.5 bg-neon-teal hover:bg-neon-teal/90 text-black text-xs h-9"
            onClick={handleJoin}
          >
            <Video size={14} />
            {booking.status === "pending"
        ...
        <button
          ...
          className="text-[11px] text-center text-muted-foreground hover:text-neon-teal transition-colors py-1"
        >
          View Full Details
        </button>
      </div>
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async () => {
      ...
      try {
        const token = await u.getIdToken(true);
        const [profile, rewardsSummary] = await Promise.all([
          getMyStudentProfile(token),
          getStudentReferralSummary(token).catch(() => null)
        ]);
        if (!cancelled) {
          setStudent(profile);
          if (rewardsSummary) setReferralRewards(rewardsSummary.referral_rewards_inr || 0);
        }
      } catch (e) {
        if (!cancelled) {
          setProfileError(
        ...
        {activeTab === "profile" && (
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              <div className="bg-background/40 rounded-2xl p-4 border border-border/50 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-neon-teal/10 flex items-center justify-center text-neon-teal">
                  <Calendar size={20} />
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Total Sessions</p>
              ...
              <div className="bg-background/40 rounded-2xl p-4 border border-border/50 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-foreground">₹{(student?.total_spent || 0).toLocaleString("en-IN")}</p>
                </div>
              </div>
              <div className="bg-background/40 rounded-2xl p-4 border border-border/50 flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-neon-teal/10 flex items-center justify-center text-neon-teal">
                ...
                <div>
                  ...
                  <p className="text-xl font-bold text-neon-teal">₹{referralRewards.toLocaleString("en-IN")}</p>
                </div>
              </div>
            </div>
